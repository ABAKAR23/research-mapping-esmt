...
<head>
    ...
    <title>Modifier le Projet - ESMT Research Mapping</title>
    ...
    <main class="content">
        <div class="form-container">
            <div class="form-header">
                ...
                <p>Modifiez les informations du projet de recherche</p>
            </div>

            <!-- Alert Container -->
            <div id="alert-container"></div>

            <form id="project-form" method="POST" action="${pageContext.request.contextPath}/api/javaee/projects/${param.id}">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="email" id="responsableEmail" name="responsableEmail" class="form-control" 
                                   placeholder="responsable@example.com" required>
...
<script>
// Get project ID from body attribute
...
async function loadProjectData() {
    try {
        ...
        if (userRole !== 'GESTIONNAIRE' && userRole !== 'ADMIN') {
            API.showError('Accès non autorisé. Seuls les Gestionnaires et Administrateurs peuvent modifier des projets.');
            setTimeout(() => {
        ...
        document.getElementById('domaineId').value = project.domaineId || '';
        document.getElementById('institution').value = project.institution || '';
        ...
        document.getElementById('budgetEstime').value = project.budgetEstime || 0;
        document.getElementById('niveauAvancement').value = project.niveauAvancement || 0;
        document.getElementById('avancementValue').textContent = (project.niveauAvancement || 0) + '%';
        document.getElementById('responsableEmail').value = project.responsableEmail || '';
        document.getElementById('participantsEmails').value = project.participantsEmails || '';
        
    } catch (error) {
        console.error('Error loading project:', error);
        API.showError('Erreur lors du chargement du projet: ' + error.message);
        setTimeout(() => {
            window.location.href = '${pageContext.request.contextPath}/projets';
        }, 2000);
    }
...
document.getElementById('project-form').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        API.showSuccess('Projet modifié avec succès ! Redirection...');
        ...
        API.showError('Erreur lors de la modification du projet: ' + error.message);
...
if (deleteBtn) {
    deleteBtn.addEventListener('click', async function() {
        if (!confirm('⚠️ Êtes-vous sûr de vouloir supprimer ce projet ? Cette action est irréversible.')) {
            return;
        }
