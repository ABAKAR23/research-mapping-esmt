...
public class StatisticsService {
     ...
     * @return Map of domain name to total budget
     */
    public Map<String, BigDecimal> getBudgetByDomain() {
        Map<String, BigDecimal> result = new HashMap<>();
        List<ResearchProject> allProjects = projectDAO.findAll();
        
        for (ResearchProject project : allProjects) {
            if (project.getDomain() != null && project.getBudgetEstimated() != null) {
                String domainName = project.getDomain().getName();
                BigDecimal currentBudget = result.getOrDefault(domainName, BigDecimal.ZERO);
                result.put(domainName, currentBudget.add(project.getBudgetEstimated()));
            }
        }
    ...
    /**
     * Get number of projects by participant
     * @return Map of participant name to project count
     */
    public Map<String, Long> getProjectsByParticipant() {
        Map<String, Long> result = new HashMap<>();
        List<ResearchProject> allProjects = projectDAO.findAll();
        
        for (ResearchProject project : allProjects) {
            if (project.getParticipations() != null) {
                for (sn.esmt.model.Participation participation : project.getParticipations()) {
                    if (participation.getUser() != null) {
                        String participantName = participation.getUser().getFirstName() + " " + 
                                               (participation.getUser().getLastName() != null ? 
                                                participation.getUser().getLastName() : "");
                        if (participantName.trim().isEmpty()) {
                            participantName = participation.getUser().getEmail();
                        }
                        result.put(participantName, result.getOrDefault(participantName, 0L) + 1);
                    }
                }
            }
        }
    ...
    /**
     * Get comprehensive statistics (all 6 required statistics)
    ...
    public Map<String, Object> getAllStatistics() {
        ...
        stats.put("budgetByDomain", getBudgetByDomain());
        stats.put("averageAdvancement", getAverageAdvancement());
        stats.put("projectsByParticipant", getProjectsByParticipant());
